	...
	WCHAR* textureFilename, int bitmapWidth, int bitmapHeight)
{
	...
	m_previousPosX = -1;  m_previousPosY = -1;
	result = InitializeBuffers(device);  if (!result) { return false; }
...
void BitmapClass::Shutdown() {  // Release the model texture.  
	ReleaseTexture(); 
...
void BitmapClass::ShutdownBuffers() {  // Release the index buffer. 
...
void BitmapClass::RenderBuffers(ID3D11DeviceContext* deviceContext) {
	...
	offset = 0;       // Set the vertex buffer to active in the input assembler so it can be rendered. 
...
void BitmapClass::ReleaseTexture() {  // Release the texture object. 
	if(m_Texture)  {   m_Texture->Shutdown();   delete m_Texture;   m_Texture = 0;  } 
	return;
}