...
DWORD WINAPI InitializeByThread(LPVOID pParam) {


	TerrainClass* thisTerrain = (TerrainClass*)pParam;

	bool result;


	// Load in the height map for the terrain.
	result = thisTerrain->LoadHeightMap(thisTerrain->heightMapFilename);
	if (!result)
	{
		return false;
	}
	...
	result = thisTerrain->LoadColorMap(thisTerrain->colorMapFilename);
	if (!result)
	{
		return false;
	}
	...
	thisTerrain->ReduceHeightMap(thisTerrain->flattenAmount);

	// Calculate the normals for the terrain data.
	result = thisTerrain->CalculateNormals();
	if (!result)
	{
		return false;
	}
	...
	thisTerrain->CalculateModelVectors();

	// Initialize the vertex and index buffer that hold the geometry for the terrain.
	result = thisTerrain->InitializeBuffers(thisTerrain->device);
	if (!result)
	{
		return false;
	}
	...
	// Load the textures.
	result = thisTerrain->LoadTextures(thisTerrain->device, thisTerrain->colorTextureFilename, thisTerrain->normalTextureFilename);
	if (!result)
	{
		return false;
	}
	...
	thisTerrain->ReleaseHeightMap();
	thisTerrain->ReleaseModel();
							  ...
							  WCHAR* normalTextureFilename)
{
	...
	this->flattenAmount = flattenAmount;
	this->colorTextureFilename = colorTextureFilename;
	this->normalTextureFilename = normalTextureFilename;

	hThread = CreateThread(NULL, 0, InitializeByThread, (LPVOID)this, 0, &dwThreadID);
