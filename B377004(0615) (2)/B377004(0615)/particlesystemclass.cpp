...
void ParticleSystemClass::EmitParticles(float frameTime)
{
	...
	if((emitParticle == true) && (m_currentParticleCount < (m_maxParticles - 1)))
	{
		...
		red = 0.8;// (((float)rand() - (float)rand()) / RAND_MAX) + 0.5f;
		green = 0.8;// (((float)rand() - (float)rand()) / RAND_MAX) + 0.5f;
		blue = 0.8;// (((float)rand() - (float)rand()) / RAND_MAX) + 0.5f;
