...
void TextClass::SetAccelation(float Accelation, ID3D11DeviceContext* deviceContext) {
	char tempString[50] = { '\0' };
	char AccelationString[50] = { '\0' };
	int INT_Accelation = (Accelation/10);
	int FLOAT_Accelation = Accelation - INT_Accelation*10;
	char tempStr[10] = { '\0' };
	
	_itoa_s(INT_Accelation, tempString, 10);
	strcat_s(tempString, " . ");
	_itoa_s(FLOAT_Accelation, tempStr, 10);

	strcat_s(tempString, tempStr);

	strcpy_s(AccelationString, "Accelation : ");
	strcat_s(AccelationString, tempString);
	strcat_s(AccelationString, "m/s");
	UpdateSentence(m_sentence1, AccelationString, 40, 20, 1.0, 1.0, 1.0, deviceContext);
...
bool TextClass::Initialize(ID3D11Device* device, ID3D11DeviceContext* deviceContext, HWND hwnd, int screenWidth, int screenHeight,
	D3DXMATRIX baseViewMatrix, char* sentence, float red, float green, float blue)
{
	bool result;
	...
	result = InitializeSentence(&m_sentence1,30, device);
	if (!result)
	{
		return false;
	}
	...
	result = UpdateSentence(m_sentence1, sentence, 100, 100, red, green, blue, deviceContext);
	if (!result)
	{
		return false;
	}
